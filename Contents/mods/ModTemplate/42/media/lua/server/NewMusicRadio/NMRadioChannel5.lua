...
function NMRadioChannel5.OnLoadRadioScripts()
	NMRadioChannel5.Init()
	table.insert(DynamicRadio.scripts, NMRadioChannel5)
end

function NMRadioChannel5.OnEveryHour(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioChannel5.CreateBroadcast(_gametime)
		_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioChannel5.OnEveryTenMinutes(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioChannel5.CreateBroadcastEveryTenMinutes(_gametime)
        	_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioChannel5.OnNewSong(_channel, _gametime, _radio, prettyName)
	local hour = _gametime:getHour()

	if hour<120 then	
		local bc = NMRadioChannel5.CreateBroadcastNewSong(_gametime, prettyName)
		_channel:setAiringBroadcast(bc)
    	end
end

Events.OnLoadRadioScripts.Add(NMRadioChannel5.OnLoadRadioScripts)

function NMRadioChannel5.CreateBroadcast(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel5.FillBroadcast(_gametime, bc)
	return bc
end

function NMRadioChannel5.CreateBroadcastEveryTenMinutes(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel5.FillBroadcastEveryTenMinutes(_gametime, bc)
	return bc
end

function NMRadioChannel5.CreateBroadcastNewSong(_gametime, prettyName)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel5.FillBroadcastNewSong(_gametime, bc, prettyName)
	return bc
end

function NMRadioChannel5.FillBroadcast(_gametime, _bc)
	local hour = _gametime:getHour()
	...
	if SandboxVars.NewMusicRadio.NMRRadioHordeNightBroadcast then
    		NMRadioChannel5.AddHordeNightNotice(c, _bc)
	end

	if SandboxVars.NewMusicRadio.NMRRadioWeatherBroadcast then
    		NMRadioChannel5.AddForecasting(c, _bc, hour)
	end

	_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )
end

function NMRadioChannel5.FillBroadcastEveryTenMinutes(_gametime, _bc)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioChannel5.FillBroadcastNewSong(_gametime, _bc, prettyName)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioChannel5.AddHordeNightNotice(_c, _bc, _force)
...
function NMRadioChannel5.AddForecasting(_c, _bc, _hour)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()

        -- forecast today and tomorrow
       	local forecast = forecaster:getForecast()
       	NMRadioChannel5.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_today"), _hour<12)

       	local forecast = forecaster:getForecast(1)
       	NMRadioChannel5.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_tomorrow"), true)

       	NMRadioChannel5.AddExtremesForecasting(_c, _bc, 2)
end

function NMRadioChannel5.AddForecast(_c, _bc, _forecast, _prefix, _doFog)
    	local s = _prefix
   	s = s .. NMRadioChannel5.GetForecastString(1, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	s = NMRadioChannel5.GetForecastString(2, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	if _doFog and _forecast:isHasFog() then
        	s = NMRadioChannel5.GetForecastString(3, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end

    	if _forecast:isWeatherStarts() then
        	-- a new weather period starts
        	s = NMRadioChannel5.GetForecastString(4, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	elseif _forecast:getWeatherOverlap() then
        	-- a already started weather period overlaps this day
        	s = NMRadioChannel5.GetForecastString(5, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end
end

function NMRadioChannel5.GetForecastString(_type, _forecast)
    	local s = ""
    	if _type==1 then
    	...
    	elseif _type==4 or _type==5 then
        	if _type==4 then
           		s = string.format(getRadioText("AEBS_weather_0_a"), NMRadioChannel5.GetDaySegmentForHour(_forecast:getWeatherStartTime()))
        	else
            		local endTime = _forecast:getWeatherEndTime()
            		if endTime>=22 then
                		s = getRadioText("AEBS_weather_0_b")
            		else
                		s = string.format(getRadioText("AEBS_weather_0_c"), NMRadioChannel5.GetDaySegmentForHour(endTime))
            		end
...
function NMRadioChannel5.GetDaySegmentForHour(_hour)
...
function NMRadioChannel5.AddExtremesForecasting(_c, _bc, offset, _len)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()
