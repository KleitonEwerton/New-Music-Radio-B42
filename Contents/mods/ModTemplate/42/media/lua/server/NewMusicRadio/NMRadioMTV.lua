...
function NMRadioMTV.OnLoadRadioScripts()
	NMRadioMTV.Init()
	table.insert(DynamicRadio.scripts, NMRadioMTV)
end

function NMRadioMTV.OnEveryHour(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioMTV.CreateBroadcast(_gametime)
		_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioMTV.OnEveryTenMinutes(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioMTV.CreateBroadcastEveryTenMinutes(_gametime)
        	_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioMTV.OnNewSong(_channel, _gametime, _radio, prettyName)
	local hour = _gametime:getHour()

	if hour<120 then	
		local bc = NMRadioMTV.CreateBroadcastNewSong(_gametime, prettyName)
		_channel:setAiringBroadcast(bc)
    	end
end

Events.OnLoadRadioScripts.Add(NMRadioMTV.OnLoadRadioScripts)

function NMRadioMTV.CreateBroadcast(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioMTV.FillBroadcast(_gametime, bc)
	return bc
end

function NMRadioMTV.CreateBroadcastEveryTenMinutes(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioMTV.FillBroadcastEveryTenMinutes(_gametime, bc)
	return bc
end

function NMRadioMTV.CreateBroadcastNewSong(_gametime, prettyName)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioMTV.FillBroadcastNewSong(_gametime, bc, prettyName)
	return bc
end

function NMRadioMTV.FillBroadcast(_gametime, _bc)
	local hour = _gametime:getHour()
	...
	if SandboxVars.NewMusicRadio.NMRRadioHordeNightBroadcast then
    		NMRadioMTV.AddHordeNightNotice(c, _bc)
	end

	--if SandboxVars.NewMusicRadio.NMRRadioWeatherBroadcast then
    	--	NMRadioMTV.AddForecasting(c, _bc, hour)
	--end

	--_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )
end

function NMRadioMTV.FillBroadcastEveryTenMinutes(_gametime, _bc)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioMTV.FillBroadcastNewSong(_gametime, _bc, prettyName)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioMTV.AddHordeNightNotice(_c, _bc, _force)
...
function NMRadioMTV.AddForecasting(_c, _bc, _hour)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()

        -- forecast today and tomorrow
       	local forecast = forecaster:getForecast()
       	NMRadioMTV.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_today"), _hour<12)

       	local forecast = forecaster:getForecast(1)
       	NMRadioMTV.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_tomorrow"), true)

       	NMRadioMTV.AddExtremesForecasting(_c, _bc, 2)
end

function NMRadioMTV.AddForecast(_c, _bc, _forecast, _prefix, _doFog)
    	local s = _prefix
   	s = s .. NMRadioMTV.GetForecastString(1, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	s = NMRadioMTV.GetForecastString(2, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	if _doFog and _forecast:isHasFog() then
        	s = NMRadioMTV.GetForecastString(3, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end

    	if _forecast:isWeatherStarts() then
        	-- a new weather period starts
        	s = NMRadioMTV.GetForecastString(4, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	elseif _forecast:getWeatherOverlap() then
        	-- a already started weather period overlaps this day
        	s = NMRadioMTV.GetForecastString(5, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end
end

function NMRadioMTV.GetForecastString(_type, _forecast)
    	local s = ""
    	if _type==1 then
    	...
    	elseif _type==4 or _type==5 then
        	if _type==4 then
           		s = string.format(getRadioText("AEBS_weather_0_a"), NMRadioMTV.GetDaySegmentForHour(_forecast:getWeatherStartTime()))
        	else
            		local endTime = _forecast:getWeatherEndTime()
            		if endTime>=22 then
                		s = getRadioText("AEBS_weather_0_b")
            		else
                		s = string.format(getRadioText("AEBS_weather_0_c"), NMRadioMTV.GetDaySegmentForHour(endTime))
            		end
...
function NMRadioMTV.GetDaySegmentForHour(_hour)
...
function NMRadioMTV.AddExtremesForecasting(_c, _bc, offset, _len)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()
