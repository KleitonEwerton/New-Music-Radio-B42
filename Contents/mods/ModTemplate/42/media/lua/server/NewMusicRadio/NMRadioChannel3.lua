...
function NMRadioChannel3.OnLoadRadioScripts()
	NMRadioChannel3.Init()
	table.insert(DynamicRadio.scripts, NMRadioChannel3)
end

function NMRadioChannel3.OnEveryHour(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioChannel3.CreateBroadcast(_gametime)
		_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioChannel3.OnEveryTenMinutes(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = NMRadioChannel3.CreateBroadcastEveryTenMinutes(_gametime)
        	_channel:setAiringBroadcast(bc)
    	end
end

function NMRadioChannel3.OnNewSong(_channel, _gametime, _radio, prettyName)
	local hour = _gametime:getHour()

	if hour<120 then	
		local bc = NMRadioChannel3.CreateBroadcastNewSong(_gametime, prettyName)
		_channel:setAiringBroadcast(bc)
    	end
end

Events.OnLoadRadioScripts.Add(NMRadioChannel3.OnLoadRadioScripts)

function NMRadioChannel3.CreateBroadcast(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel3.FillBroadcast(_gametime, bc)
	return bc
end

function NMRadioChannel3.CreateBroadcastEveryTenMinutes(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel3.FillBroadcastEveryTenMinutes(_gametime, bc)
	return bc
end

function NMRadioChannel3.CreateBroadcastNewSong(_gametime, prettyName)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	NMRadioChannel3.FillBroadcastNewSong(_gametime, bc, prettyName)
	return bc
end

function NMRadioChannel3.FillBroadcast(_gametime, _bc)
	local hour = _gametime:getHour()
	...
	if SandboxVars.NewMusicRadio.NMRRadioHordeNightBroadcast then
    		NMRadioChannel3.AddHordeNightNotice(c, _bc)
	end

	if SandboxVars.NewMusicRadio.NMRRadioWeatherBroadcast then
    		NMRadioChannel3.AddForecasting(c, _bc, hour)
	end

	_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )
end

function NMRadioChannel3.FillBroadcastEveryTenMinutes(_gametime, _bc)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioChannel3.FillBroadcastNewSong(_gametime, _bc, prettyName)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function NMRadioChannel3.AddHordeNightNotice(_c, _bc, _force)
...
function NMRadioChannel3.AddForecasting(_c, _bc, _hour)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()

        -- forecast today and tomorrow
       	local forecast = forecaster:getForecast()
       	NMRadioChannel3.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_today"), _hour<12)

       	local forecast = forecaster:getForecast(1)
       	NMRadioChannel3.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_tomorrow"), true)

       	NMRadioChannel3.AddExtremesForecasting(_c, _bc, 2)
end

function NMRadioChannel3.AddForecast(_c, _bc, _forecast, _prefix, _doFog)
    	local s = _prefix
   	s = s .. NMRadioChannel3.GetForecastString(1, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	s = NMRadioChannel3.GetForecastString(2, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	if _doFog and _forecast:isHasFog() then
        	s = NMRadioChannel3.GetForecastString(3, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end

    	if _forecast:isWeatherStarts() then
        	-- a new weather period starts
        	s = NMRadioChannel3.GetForecastString(4, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	elseif _forecast:getWeatherOverlap() then
        	-- a already started weather period overlaps this day
        	s = NMRadioChannel3.GetForecastString(5, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end
end

function NMRadioChannel3.GetForecastString(_type, _forecast)
    	local s = ""
    	if _type==1 then
    	...
    	elseif _type==4 or _type==5 then
        	if _type==4 then
           		s = string.format(getRadioText("AEBS_weather_0_a"), NMRadioChannel3.GetDaySegmentForHour(_forecast:getWeatherStartTime()))
        	else
            		local endTime = _forecast:getWeatherEndTime()
            		if endTime>=22 then
                		s = getRadioText("AEBS_weather_0_b")
            		else
                		s = string.format(getRadioText("AEBS_weather_0_c"), NMRadioChannel3.GetDaySegmentForHour(endTime))
            		end
...
function NMRadioChannel3.GetDaySegmentForHour(_hour)
...
function NMRadioChannel3.AddExtremesForecasting(_c, _bc, offset, _len)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()
